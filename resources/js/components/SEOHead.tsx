...
export default function SEOHead({ title, description, canonicalUrl, robots, noindex, og, twitter, children }: SEOHeadProps) {
    const appName = (import.meta.env.VITE_APP_NAME as string) || 'Modulo CMS';
    const pageTitle = title ?? appName;

    useDocumentTitle(pageTitle);

    const canonical = canonicalUrl || (typeof window !== 'undefined' ? `${window.location.origin}${window.location.pathname}` : undefined);
    ...
    return (
        <Head title={pageTitle}>
            ...
            {ogSiteName && <meta key="og:site_name" property="og:site_name" content={ogSiteName} />}
            ...
            {twDescription && <meta key="twitter:description" name="twitter:description" content={twDescription} />}
