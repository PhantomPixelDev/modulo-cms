                ...
                {(isAdmin() || hasPermission('edit menus')) && (
                    <Card className="border-border/60 shadow-none xl:col-span-2">
                        <CardHeader className="space-y-1">
                            ...
                            <CardDescription className="text-sm text-muted-foreground">Update the menu’s metadata and description.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <form onSubmit={submit} className="space-y-4">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Location</label>
                                ...
                                <div className="flex items-center justify-end gap-2">
                                    <Button
                                        ...
                                        variant="outline"
                                        size="sm"
                                        onClick={() => router.reload({ only: ['menu'] })}
                                        disabled={processing}
                                    >
                                        Reset
                                    </Button>
...
function CreateItemForm({ menuId, allItems, locales }: { menuId: number; allItems: MenuItemDTO[]; locales: Locale[] }) {
    ...
    return (
        <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-1.5">
                <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Link Type</label>
                <select
            ...
            {linkType === 'url' && (
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">URL</label>
            ...
            {linkType === 'page' && (
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Page Slug</label>
                    <Input placeholder="about, contact" value={data.page_slug} onChange={(e) => setData('page_slug', e.target.value)} />
                </div>
            )}
            {linkType === 'route' && (
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Route Name</label>
                    <Input
                        placeholder="dashboard.admin.posts.index"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Target</label>
                    <select
                        className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
                        value={data.target}
                        onChange={(e) => setData('target', e.target.value as '_self' | '_blank')}
                    >
                ...
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Visible To</label>
                    <select
                        className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
                        value={data.visible_to}
                        onChange={(e) => setData('visible_to', e.target.value as 'all' | 'guest' | 'auth')}
                    >
                ...
                <div className="space-y-1.5">
                    <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Parent</label>
                    <select
                        className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-ring focus:outline-none"
            ...
            {locales.length > 0 && (
                <div className="space-y-2">
                    <label className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Localized Labels & URLs</label>
            ...
            <div className="flex items-center justify-end gap-2">
                <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => reset('label', 'url', 'page_slug', 'route_name', 'parent_id', 'order')}
                    disabled={processing}
                >
                    Reset
                </Button>
...
function ItemRow({
    ...
    const parentOptions = useMemo(
        () => [
            { id: 0, label: '— Root —' },
            ...allItems.filter((i) => i.id !== item.id && !descendantIds.has(i.id)).map((i) => ({ id: i.id, label: i.label })),
        ],
