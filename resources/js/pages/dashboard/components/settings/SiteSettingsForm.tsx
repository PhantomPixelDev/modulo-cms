...
export function SiteSettingsForm({
    ...
    const handleTabChange = (value: string): void => {
        ...
        router.get(
            ...
            { group: value, locale: selectedLocale },
            {
                preserveState: true,
                preserveScroll: true,
                replace: true,
    ...
    const handleLocaleSwitch = (localeCode: string) => {
        ...
        router.get(
            '/dashboard/admin/settings',
            { group: activeTab, locale: localeCode },
            {
                preserveState: true,
                preserveScroll: true,
                replace: true,
    ...
    const renderReadingSettings = () => (
        <div className="space-y-6">
            ...
            {formData.reading?.show_on_front === 'page' && (
                <div className="grid gap-6 rounded-lg border border-dashed bg-muted/20 p-4 md:grid-cols-2">
                    <div className="space-y-2">
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 rounded-lg border bg-muted/5 p-4 sm:grid-cols-2 md:grid-cols-3">
                        {commonMimeTypes.map((mime) => (
                            <div key={mime.value} className="flex items-center space-x-2">
                                <Switch
                                    ...
                                    disabled={!canEdit}
                                />
                                <Label htmlFor={`mime-${mime.value}`} className="cursor-pointer text-xs font-medium">
                                    {mime.label}
                                </Label>
                            </div>
                        ))}
    ...
    const renderPermalinksSettings = () => (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-2">
                        <Label htmlFor="category_base" className="text-sm font-bold">
                            {t('dashboard.settings.permalinks.category_base')}
                        </Label>
                        <Input
                            id="category_base"
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="tag_base" className="text-sm font-bold">
                            {t('dashboard.settings.permalinks.tag_base')}
                        </Label>
                        <Input
                            id="tag_base"
    ...
    const renderAdvancedSettings = () => (
        <div className="space-y-6">
            <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between rounded-lg border bg-muted/5 p-4">
                ...
                {formData.advanced?.maintenance_mode && (
                    <div className="space-y-2 duration-200 animate-in fade-in slide-in-from-top-2">
                        <Label htmlFor="maintenance_message" className="text-sm font-bold">
                            {t('dashboard.settings.advanced.maintenance_message')}
                        </Label>
                        <Textarea
        ...
        <div className="space-y-6">
            <Card className="overflow-hidden border-border shadow-sm">
                {availableLocales.length > 1 && (
                    <div className="flex items-center justify-end gap-3 border-b bg-muted/20 px-6 py-3">
                                ...
                                {availableLocales.map((locale) => (
                                    <SelectItem key={locale.code} value={locale.code}>
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            {locale.native_name && <span className="text-xs text-muted-foreground">{locale.native_name}</span>}
                                        </div>
                                    </SelectItem>
                                ))}
                            </SelectContent>
                            ...
                            <TabsList className="h-12 gap-4 bg-transparent p-0">
                                {groups.map((group) => (
                                    <TabsTrigger
                                        key={group}
                                        value={group}
                                        className="h-12 rounded-none border-b-2 border-transparent px-2 text-xs font-semibold tracking-wider uppercase transition-all hover:text-foreground/80 data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none"
                                    >
                                        <div className="flex items-center gap-2">
                                            {groupIcons[group]}
                            ...
                            <TabsContent value="general" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('general')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_general')}
                                    </Button>
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="reading" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('reading')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_reading')}
                                    </Button>
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="writing" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('writing')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_writing')}
                                    </Button>
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="permalinks" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('permalinks')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_permalinks')}
                                    </Button>
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="seo" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('seo')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_seo')}
                                    </Button>
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="social" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('social')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_social')}
                                    </Button>
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="analytics" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('analytics')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_analytics')}
                                    </Button>
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="media" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('media')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_media')}
                                    </Button>
                                </div>
                            </TabsContent>
                            ...
                            <TabsContent value="advanced" className="mt-0 space-y-6 focus-visible:outline-none">
                                ...
                                <div className="flex justify-end border-t pt-4">
                                    <Button onClick={() => handleSave('advanced')} disabled={!canEdit || saving} className="px-8">
                                        {saving ? t('dashboard.settings.actions.saving') : t('dashboard.settings.actions.save_advanced')}
                                    </Button>
                                </div>
                            </TabsContent>
                        </div>
