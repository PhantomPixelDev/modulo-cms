...
export function PostList({ posts, locales = [], canCreate = false, canEdit = false, onCreate }: PostListProps) {
                                    ...
                                    <button
                                        ...
                                        className={`inline-flex h-6 min-w-6 items-center justify-center rounded px-1.5 text-xs font-medium transition-colors ${
                                            hasTranslation
                                                ...
                                                : 'border border-dashed bg-muted text-muted-foreground hover:bg-muted/80'
                                        } `}
                                    >
    ...
    const columns = [
        ...
        {
            key: 'post_type',
            label: t('dashboard.posts.post_type'),
            sortable: false,
            render: (item: PostListItem) => <Badge variant="outline">{item.post_type?.label || item.post_type?.name || '—'}</Badge>,
        },
        {
            key: 'status',
        ...
        {
            key: 'created_at',
            label: t('dashboard.posts.post_date'),
            sortable: true,
            render: (item: PostListItem) => new Date(item.created_at).toLocaleDateString(),
        },
    ];
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Data Table */}
            <DataTable data={filteredItems} columns={columns} actions={actions} itemsPerPage={15} searchFields={[]} />
        </div>
    );
}
