...
export function PostTypeList({ items, canView = false, canEdit = false, canDelete = false, onView, onEdit, onDelete }: PostTypeListProps) {
    if (!items || items.length === 0) {
        return <EmptyState title="No post types" description="Create a post type to get started." />;
    }
    ...
    return <DataTable data={items} columns={columns} actions={actions} itemsPerPage={10} searchFields={['name', 'label', 'route_prefix']} />;
}
