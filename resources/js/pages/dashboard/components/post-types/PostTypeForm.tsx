                ...
                <CardContent className="grid gap-4 p-4 md:p-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <label className="flex items-center gap-2 text-sm">
                                    <Checkbox checked={form.is_public} onCheckedChange={(c) => handleChange('is_public', Boolean(c))} />{' '}
                                    {t('dashboard.post_types.form.options_list.public')}
                                </label>
                                <label className="flex items-center gap-2 text-sm">
                                    <Checkbox checked={form.is_hierarchical} onCheckedChange={(c) => handleChange('is_hierarchical', Boolean(c))} />{' '}
                                    {t('dashboard.post_types.form.options_list.hierarchical')}
                                </label>
                                <label className="flex items-center gap-2 text-sm">
                                    <Checkbox checked={form.show_in_menu} onCheckedChange={(c) => handleChange('show_in_menu', Boolean(c))} />{' '}
                                    {t('dashboard.post_types.form.options_list.show_in_menu')}
                                </label>
                                <label className="flex items-center gap-2 text-sm">
                                    <Checkbox checked={form.has_taxonomies} onCheckedChange={(c) => handleChange('has_taxonomies', Boolean(c))} />{' '}
                                    {t('dashboard.post_types.form.options_list.has_taxonomies')}
                                </label>
                                <label className="flex items-center gap-2 text-sm">
                                    <Checkbox
                                        checked={form.has_featured_image}
                                        onCheckedChange={(c) => handleChange('has_featured_image', Boolean(c))}
                                    />{' '}
                                    {t('dashboard.post_types.form.options_list.has_featured_image')}
                                </label>
                                <label className="flex items-center gap-2 text-sm">
                                    <Checkbox checked={form.has_excerpt} onCheckedChange={(c) => handleChange('has_excerpt', Boolean(c))} />{' '}
                                    {t('dashboard.post_types.form.options_list.has_excerpt')}
                                </label>
                                <label className={cn('flex items-center gap-2 text-sm', !globalCommentsEnabled && 'cursor-not-allowed opacity-50')}>
                                    <Checkbox
                                    ...
                                    {!globalCommentsEnabled && (
                                        <span className="ml-1 text-[10px] text-muted-foreground">
                                            {t('dashboard.post_types.form.comments_disabled')}
                                        </span>
                                    )}
                                </label>
                            </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            ...
                            />
                            <p className="mt-1 text-xs text-muted-foreground">{t('dashboard.post_types.form.hints.taxonomies')}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <Input
                                ...
                                onChange={(e) => handleChange('menu_position', Number(e.target.value))}
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2">
                        <Button type="button" variant="outline" onClick={onCancel}>
                            {t('dashboard.post_types.form.buttons.cancel')}
                        </Button>
                        <Button type="submit">
                            {isEditing ? t('dashboard.post_types.form.buttons.update') : t('dashboard.post_types.form.buttons.create')}
                        </Button>
                    </div>
                </CardContent>
            </Card>
