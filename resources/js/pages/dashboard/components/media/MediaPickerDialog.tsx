...
export const MediaPickerDialog: React.FC<MediaPickerDialogProps> = ({ open, onOpenChange, onSelect, initialFolderId = null, type = 'image' }) => {
    ...
    const [pagination, setPagination] = useState<Pick<Paginated<MediaItem>, 'current_page' | 'last_page' | 'per_page' | 'total'>>({
        current_page: 1,
        last_page: 1,
        per_page: 24,
        total: 0,
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-5xl">
                ...
                <div className="space-y-3">
                    ...
                    {/* Media grid */}
                    <div className="grid min-h-24 grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                        {items.map((m) => (
                            <button
                                key={m.id}
                                type="button"
                            ...
                            >
                                <div className="flex aspect-square items-center justify-center overflow-hidden rounded bg-muted">
                                    <img src={m.thumb || m.url} alt={m.custom_properties?.alt || m.name} className="h-full w-full object-cover" />
                                </div>
                                <div className="text-xs break-words">
                                    <div className="truncate font-medium" title={m.name}>
                                        {m.name}
                                    </div>
                                    <div className="truncate text-muted-foreground" title={m.mime_type}>
                                        {m.mime_type}
                                    </div>
                                </div>
                            </button>
                    ...
                    {pagination && pagination.last_page > 1 && (
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <div className="flex items-center gap-2">
                                <Button size="sm" variant="outline" disabled={!canPrev} onClick={() => load(pagination.current_page - 1)}>
                                    Prev
                                </Button>
                                <Button size="sm" variant="outline" disabled={!canNext} onClick={() => load(pagination.current_page + 1)}>
                                    Next
                                </Button>
                            </div>
                        </div>
