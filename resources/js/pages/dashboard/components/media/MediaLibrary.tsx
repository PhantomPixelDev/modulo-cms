...
export const MediaLibrary: React.FC<Props> = ({
    ...
    const filteredAllFolders = useMemo(() => {
        ...
        if (!term) return list;
        return list.filter((f) => (f.name && f.name.toLowerCase().includes(term)) || (f.path && f.path.toLowerCase().includes(term)));
    ...
    return (
        <div className="space-y-4" onDrop={handleDrop} onDragOver={handleDragOver} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave}>
                ...
                <div className="flex flex-wrap items-center gap-1 text-sm text-muted-foreground">
                    {breadcrumb.map((b, idx) => (
                        <span key={b.id} className="flex items-center gap-1">
                            <button type="button" className="underline hover:no-underline" onClick={() => navigateToFolder(b.id)}>
                                {b.name}
                            </button>
            ...
            <div className="flex flex-wrap items-center gap-3">
                {canUpload && (
                    <div className="flex items-center gap-3">
                        <input type="file" ref={fileRef} className="hidden" multiple onChange={onFileInputChange} />
                        <Button size="sm" onClick={() => fileRef.current?.click()} disabled={uploading}>
                            {uploading && uploadTotal > 0 ? `Uploading ${uploadCount}/${uploadTotal}…` : 'Select files…'}
                        </Button>
                        <span className="hidden text-xs text-muted-foreground sm:inline">or drag & drop into the area below</span>
                    </div>
                )}
                ...
                <div className="ml-auto flex items-center gap-2">
                    ...
                    <Button size="sm" variant="secondary" onClick={doSearch}>
                        Apply
                    </Button>
                </div>
            </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {selected.size > 0 && (
                        <div className="flex items-center gap-2">
                            ...
                            {canEdit && (
                                <>
                                    <Button size="sm" onClick={() => doBulk('regenerate')}>
                                        Regenerate
                                    </Button>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {items.map((m) => (
                    <div key={m.id} className="flex flex-col gap-2 rounded border p-2">
                        <div className="flex items-center justify-between text-xs">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    onClick={() => {
                                        try {
                                            (navigator as any)?.clipboard?.writeText?.(m.url);
                                        } catch {
                                            void 0;
                                        }
                                    }}
                ...
                {items.length === 0 && <div className="col-span-full text-sm text-muted-foreground">No media yet.</div>}
            </div>
