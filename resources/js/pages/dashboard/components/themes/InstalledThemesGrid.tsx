...
export function InstalledThemesGrid({
    ...
    onPublishAssets,
    onUninstall,
}: InstalledThemesGridProps) {
    return (
        <div>
            <h3 className="mb-3 font-semibold">Installed Themes</h3>
                        ...
                        <div key={t.id} className={`rounded-xl border bg-card p-4 shadow-xs ${isActive ? 'border-primary ring-1 ring-primary' : ''}`}>
                            <div className="flex items-start justify-between">
                                    ...
                                    <div>
                                        ...
                                        {(cfg?.version || cfg?.author) && (
                                            <div className="mt-1 text-xs text-muted-foreground">
                                                ...
                                                {cfg?.author &&
                                                    (cfg?.author_url ? (
                                                        <a className="underline" href={cfg.author_url} target="_blank" rel="noreferrer">
                                                            {cfg.author}
                                                        </a>
                                                    ) : (
                                ...
                                <div className="flex gap-2">
                                    <Button variant="outline" size="sm" onClick={() => onView?.(t.id)}>
                                        View
                                    </Button>
                                </div>
                            </div>
