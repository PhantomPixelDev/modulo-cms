...
export function DiscoveredThemesList({ items }: DiscoveredThemesListProps) {
    return (
        <div>
            <h3 className="mb-3 font-semibold">Discovered Themes</h3>
            {items.length === 0 ? (
                <div className="text-sm text-muted-foreground">No uninstalled themes discovered.</div>
            ) : (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {items.map((t) => (
                        <div key={t.config?.slug || t.slug} className="flex items-center justify-between rounded-md border p-4">
                            <div>
                                ...
                                {(t?.config?.version || t?.config?.author) && (
                                    <div className="mt-1 text-xs text-muted-foreground">
                                        ...
                                        {t?.config?.author &&
                                            (t?.config?.author_url ? (
                                                <a className="underline" href={t.config.author_url} target="_blank" rel="noreferrer">
                                                    {t.config.author}
                                                </a>
                                            ) : (
