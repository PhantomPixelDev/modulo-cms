...
const createInitialState = (taxonomy?: Partial<Taxonomy> | null): FormState => ({
    ...
    post_types: Array.isArray(taxonomy?.post_types) ? taxonomy!.post_types : [],
});
            ...
            <Card className="rounded-md border border-border/60 shadow-none">
                <CardContent className="grid gap-4 p-4 sm:p-5">
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            <label className="flex items-center gap-2 text-sm">
                                <Checkbox checked={form.is_public} onCheckedChange={(checked) => handleChange('is_public', Boolean(checked))} />
                                <span>Public</span>
                            </label>
                            <label className="flex items-center gap-2 text-sm">
                                <Checkbox
                            ...
                            <label className="flex items-center gap-2 text-sm">
                                <Checkbox checked={form.show_in_menu} onCheckedChange={(checked) => handleChange('show_in_menu', Boolean(checked))} />
                            ...
                            {postTypeOptions.length === 0 ? (
                                ...
                                postTypeOptions.map((pt) => {
                                    ...
                                    return (
                                        <label
                                            key={pt.id}
                                            className="flex items-center gap-2 rounded border border-border/60 bg-muted/30 px-3 py-2 text-sm"
                                        >
                                            <Checkbox checked={checked} onCheckedChange={(value) => togglePostType(identifier, Boolean(value))} />
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2">
                        ...
                        <Button type="submit">{isEditing ? 'Update Taxonomy' : 'Create Taxonomy'}</Button>
                    </div>
                </CardContent>
            </Card>
        </form>
