...
export function TaxonomyList({ items, canView = false, canEdit = false, onView, onEdit }: TaxonomyListProps) {
    ...
    if (!items || items.length === 0) {
        return <EmptyState title="No taxonomies" description="Create a taxonomy to get started." />;
    }

    return <DataTable data={items} columns={columns} actions={actions} itemsPerPage={10} searchFields={['name', 'label']} />;
}
