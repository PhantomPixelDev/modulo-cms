...
export function SitemapSettingsForm({
    ...
    const sortedPublic = useMemo(() => [...postTypes].sort((a, b) => (a.menu_position ?? 0) - (b.menu_position ?? 0)), [postTypes]);
    ...
    const handleLocaleSwitch = (localeCode: string) => {
        ...
        router.get(ROUTE.sitemap.index(), { locale: localeCode }, { preserveState: true, preserveScroll: true, replace: true });
    };
    ...
    const sanitizeCustomUrls = (): SitemapCustomUrl[] =>
        customUrls
            .map((entry) => ({
                ...
                priority:
                    entry.priority === undefined || entry.priority === null || entry.priority === ('' as any) ? undefined : Number(entry.priority),
            }))
    ...
    return (
        <div className="space-y-6">
            {locales.length > 1 && (
                <div className="flex items-center justify-end gap-3 rounded-md border bg-muted/30 p-3">
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3">
                {sortedPublic.map((pt) => (
                    <label key={pt.id} className="flex items-center gap-2 rounded border p-2">
                        <input type="checkbox" checked={included.has(pt.id)} onChange={() => toggle(pt.id)} disabled={!canEdit} />
            ...
            <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                    <label className="w-40 text-sm">{t('dashboard.sitemap.fields.cache_ttl')}</label>
                    <input
                        type="number"
                        min={60}
                        max={86400}
