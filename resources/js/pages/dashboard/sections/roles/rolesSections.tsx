...
import { RoleForm, RoleList } from '../../components/roles';
    ...
    const handleRoleSubmit = async (data: any) => {
        ...
        try {
            await router[method](url, data, {
                preserveScroll: true,
                onSuccess: () => {
                    showSuccess(t(editRole ? 'dashboard.roles.messages.updated' : 'dashboard.roles.messages.created'));
                    router.visit(ROUTE.roles.index());
                },
                onError: (errors) => {
                    console.error('Failed to save role:', errors);
                    showError(t(editRole ? 'dashboard.roles.messages.update_failed' : 'dashboard.roles.messages.create_failed'));
                },
            });
        } catch (error) {
