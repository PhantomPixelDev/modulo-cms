    ...
    const handleUserSubmit = async (formData: any) => {
        try {
            ...
            await router[method](url, formData, {
                preserveScroll: true,
                onSuccess: () => {
                    showSuccess(t(editUser ? 'dashboard.users.messages.updated' : 'dashboard.users.messages.created'));
                    router.visit(ROUTE.users.index());
                },
                onError: (errors) => {
                    console.error('Validation errors:', errors);
                    showError(t(editUser ? 'dashboard.users.messages.update_failed' : 'dashboard.users.messages.create_failed'));
                },
            });
        } catch (error) {
        ...
        >
            <UserList
                ...
                onRoleChange={async (userId: number, roleId: number, action: 'assign' | 'remove') => {
                    try {
                        const url = action === 'assign' ? ROUTE.users.roles.assign(userId, roleId) : ROUTE.users.roles.remove(userId, roleId);
                        await router.post(
                            url,
                            {},
                            {
                                ...
                                onSuccess: () => {
                                    showSuccess(
                                        t(action === 'assign' ? 'dashboard.users.messages.role_assigned' : 'dashboard.users.messages.role_removed'),
                                    );
        ...
        if (!editUser) {
            return (
                <SectionWrapper title={t('dashboard.users.not_found.title')}>
                    ...
                    <Button variant="outline" size="sm" onClick={() => router.visit(ROUTE.users.index())} className="mt-4">
                        {t('dashboard.users.actions.back')}
                    </Button>
                </SectionWrapper>
            );
