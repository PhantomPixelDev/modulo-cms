...
export function getTaxonomiesSections({
    ...
    const renderTaxonomyForm = () => (
        <SectionWrapper
            title={adminSection === 'taxonomies.create' ? t('dashboard.taxonomies.create_title') : t('dashboard.taxonomies.edit_title')}
            description={
                adminSection === 'taxonomies.create' ? t('dashboard.taxonomies.create_description') : t('dashboard.taxonomies.edit_description')
            }
            actions={
                ...
                onSubmit={async (payload) => {
                    ...
                    try {
                        ...
                        await router[method](url, payload, {
                            preserveScroll: true,
                            onSuccess: () => {
                                showSuccess(t(isEditingTaxonomy ? 'dashboard.taxonomies.messages.updated' : 'dashboard.taxonomies.messages.created'));
                        ...
                        showError(
                            t(isEditingTaxonomy ? 'dashboard.taxonomies.messages.update_failed' : 'dashboard.taxonomies.messages.create_failed'),
                        );
                    }
                }}
