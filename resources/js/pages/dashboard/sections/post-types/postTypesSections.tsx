    ...
    const handlePostTypeSubmit = async (formData: any) => {
        try {
            ...
            await router[method](url, formData, {
                preserveScroll: true,
                onSuccess: () => {
                    showSuccess(t((editPostType as any) ? 'dashboard.post_types.messages.updated' : 'dashboard.post_types.messages.created'));
                    router.visit(ROUTE.postTypes.index());
                },
                onError: (errors) => {
                    console.error('Validation errors:', errors);
                    showError(
                        t((editPostType as any) ? 'dashboard.post_types.messages.update_failed' : 'dashboard.post_types.messages.create_failed'),
                    );
                },
            });
        } catch (error) {
            console.error('Error saving post type:', error);
            showError(t((editPostType as any) ? 'dashboard.post_types.messages.update_failed' : 'dashboard.post_types.messages.create_failed'));
        }
    };
