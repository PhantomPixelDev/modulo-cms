...
export default function DashboardContent({
    ...
    const renderSection = () => {
        ...
        const sectionsMap: Record<string, () => ReactNode> = {
            ...getSystemSections({ updateCenter, backups, activity, redirects, mailSettings }),
    ...
    const getPageTitle = (): string => {
        ...
        if (item) {
            ...
            const type =
                editUser || (item as any)?.name?.includes('user')
                    ? 'User'
                    : editRole || (item as any)?.name?.includes('role')
                      ? 'Role'
                      : editPost || (item as any)?.title
                        ? 'Post'
                        : editPostType
                          ? 'Post Type'
                          : editTaxonomy
